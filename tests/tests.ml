...
let test_attack_cell _ =
  let test_board =
    ...
    | None -> B.create_board
  in
  let board_option, attack_result = B.attack_cell test_board (0, 'A') in
  let result_board =
    match board_option with Some board -> board | None -> B.create_board
  in
  assert_equal
    (" H(0,A)  O(0,B)  O(0,C)  O(0,D)  O(0,E) "
   ^ " E(0,F)  E(0,G)  E(0,H)  E(0,I)  E(0,J) \n"
   ^ " E(1,A)  E(1,B)  E(1,C)  E(1,D)  E(1,E) "
   ^ " E(1,F)  E(1,G)  E(1,H)  E(1,I)  E(1,J) \n"
   ^ " E(2,A)  E(2,B)  E(2,C)  E(2,D)  E(2,E) "
